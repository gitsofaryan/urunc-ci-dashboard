...
// of recent CI results for urunc.
//
// The general flow is as follows:
//   - It queries the GitHub API for the workflow runs for the CI (e.g.
//     the last 10 runs).
//   - For each of those runs, it queries the API for all the jobs data.
...
// GitHub API URL for the urunc CI workflow runs.
// Will only get the most recent 10 runs ('per_page=10').
const total_runs = 10;

const ci_workflow_runs_url =
  "https://api.github.com/repos/" +
  "urunc-dev/urunc/actions/workflows/" +
  `ci.yml/runs?per_page=${total_runs}`;

// GitHub API URL for the main branch of the urunc repo.
// Used to get the list of required jobs.
const main_branch_url =
  "https://api.github.com/repos/" + "urunc-dev/urunc/branches/main";
...
function get_required_jobs(main_branch) {
  try {
    return (
      main_branch["protection"]["required_status_checks"]["contexts"] || []
    );
  } catch (e) {
    return [];
  }
...
function get_job_data(run) {
  ...
  async function fetch_jobs_by_page(which_page) {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch jobs from ${jobs_url}: ${response.status}: ` +
          `${response.statusText}`,
      );
    }
    const json = await response.json();
...
async function main() {
  ...
  const workflow_runs = await fetch_url(ci_workflow_runs_url);

  // Fetch required jobs from main branch
  let required_jobs = [];
  try {
    const main_branch = await fetch_url(main_branch_url);
    required_jobs = get_required_jobs(main_branch);
  } catch (e) {
    console.error("Could not fetch required jobs:", e.message);
  }
